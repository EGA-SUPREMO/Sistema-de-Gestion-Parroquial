...
<body class="d-flex flex-column min-vh-100 bg-light">
    ...
    <header class="bg-success text-white text-center py-3">
        <h1 class="mb-0">Gestión de Feligreses</h1>
    </header>
                ...
                <section class="card shadow-lg rounded-4">
                    <div class="card-body p-4 p-md-5">
                        ...
                        <div class="d-flex justify-content-end mb-4">
                            <a href="?c=feligreses&a=Registro" class="btn btn-primary btn-lg rounded-pill shadow-sm">
                               Añadir Nuevo Feligrés
                            </a>
                        </div>

                        <?php if (empty($feligreses)): ?>
                            <div class="alert alert-info text-center py-4 rounded-3" role="alert">
                                <i class="bi bi-info-circle-fill me-2"></i> No hay feligreses registrados aún.
                                <p class="mt-2">¡Haz clic en "Añadir Nuevo Feligrés" para empezar!</p>
                            </div>
                        <?php else: ?>
                                ...
                                <table class="table table-striped table-hover align-middle border rounded-3 overflow-hidden">
                                    <thead class="table-success text-white">
                                        <tr>
                                            ...
                                            <th scope="col" class="py-3 px-4">Cédula</th>
                                            <th scope="col" class="py-3 px-4 text-center">Editar - Eliminar</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <?php foreach ($feligreses as $f): ?>
                                            <tr>
                                                <th scope="row" class="px-4"><?= htmlspecialchars($f->id) ?></th>
                                                <td class="px-4"><?= htmlspecialchars($f->nombre) ?></td>
                                                <td class="px-4"><?= htmlspecialchars($f->cedula) ?></td>
                                                <td class="text-center px-4">
                                                    <a href="?c=feligreses&a=Editar&id=<?= htmlspecialchars($f->id) ?>" class="btn btn-sm btn-warning me-2 rounded-pill shadow-sm">
                                                      Editar
                                                    </a>

                                                    <a class="btn btn-danger zoom-out rounded-pill shadow-sm"
                                                        onclick="javascript:return confirm('¿Seguro de eliminar a <?= htmlspecialchars($f->nombre) ?>?');"
                                                        href="?c=feligreses&a=Eliminar&id=<?= htmlspecialchars($f->id) ?>">
                                                        Eliminar
                                                    </a>
                                                </td>
                                            </tr>
